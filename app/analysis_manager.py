...
class AnalysisManager:
    ...
    # Stamp Catalog Database removed — functionality was not viable
    # due to catalog system complexity and translation issues.
    ...
    def open_spectral_analysis(self):
        ...
        try:
            ...
            if not measurements and hasattr(self.app.canvas, '_coord_markers') and self.app.canvas._coord_markers:
                ...
                if current_sample_set:
                    error_msg += f"• Searched for sample set: '{current_sample_set}'\\n"
                ...
                try:
                    ...
                    available_databases = ColorAnalysisDB.get_all_sample_set_databases(get_color_analysis_dir())
                    if available_databases:
                        error_msg += f"• Available databases: {', '.join(available_databases[:3])}" + ("..." if len(available_databases) > 3 else "") + "\\n"
                    else:
                        error_msg += "• No databases found in Application Support\\n"
                except:
                    error_msg += "• Could not check available databases\\n"
                    
                error_msg += f"• Sample markers on canvas: {len(self.app.canvas._coord_markers)}"
                
                messagebox.showwarning("No Analysis Data", error_msg)
                return
