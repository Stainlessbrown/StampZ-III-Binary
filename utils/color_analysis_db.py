...
class ColorAnalysisDB:
    ...
    def _init_db(self):
        ...
        with sqlite3.connect(self.db_path) as conn:
            ...
            # Add L*a*b* standard deviation columns
            try:
                cursor.execute("ALTER TABLE color_measurements ADD COLUMN lab_l_stddev REAL")
                print("Added lab_l_stddev column")
            except sqlite3.OperationalError:
                pass  # Column already exists
            
            try:
                cursor.execute("ALTER TABLE color_measurements ADD COLUMN lab_a_stddev REAL")
                print("Added lab_a_stddev column")
            except sqlite3.OperationalError:
                pass  # Column already exists
            
            try:
                cursor.execute("ALTER TABLE color_measurements ADD COLUMN lab_b_stddev REAL")
                print("Added lab_b_stddev column")
            except sqlite3.OperationalError:
                pass  # Column already exists
    ...
    def save_color_measurement(
        ...
        lab_l_stddev: Optional[float] = None,
        lab_a_stddev: Optional[float] = None,
        lab_b_stddev: Optional[float] = None
    ) -> bool:
