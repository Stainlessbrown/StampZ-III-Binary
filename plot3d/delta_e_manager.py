...
class DeltaEManager:
    ...
    def _get_row_indices(self, start: int, end: int) -> range:
        ...
        For realtime mode, use the _original_sheet_row mapping.
        For file mode, use simple row-2 conversion.
        
        Args:
            start: 1-based start row number (display row numbers)
            end: 1-based end row number (display row numbers)
        Returns:
            range object with 0-based DataFrame indices
        """
        # Validate and get proper bounds
        start, end = self.validate_row_range(start, end)
        
        # Check if we're in realtime mode (has _original_sheet_row column)
        if self.data is not None and '_original_sheet_row' in self.data.columns:
            self.logger.info(f"🔍 Using realtime row mapping for rows {start}-{end}")
            
            # Find DataFrame indices that correspond to the requested display rows
            # Convert display rows to 0-based sheet rows (display 8 = sheet row 7)
            sheet_start = start - 1  # Display row 8 = sheet row 7
            sheet_end = end - 1      # Display row 36 = sheet row 35
            
            # Find DataFrame indices where _original_sheet_row is in our range
            matching_indices = []
            for df_idx, row in self.data.iterrows():
                orig_sheet_row = row.get('_original_sheet_row')
                if pd.notna(orig_sheet_row) and sheet_start <= orig_sheet_row <= sheet_end:
                    matching_indices.append(df_idx)
            
            self.logger.info(f"🔍 Found {len(matching_indices)} DataFrame indices for display rows {start}-{end}: {matching_indices[:5]}{'...' if len(matching_indices) > 5 else ''}")
            
            if matching_indices:
                # Return the matching indices as a range-like object
                return matching_indices
            else:
                self.logger.warning(f"No DataFrame indices found for display rows {start}-{end}")
                return []
        else:
            # File mode - use original logic
            self.logger.info(f"🔍 Using file-based row mapping for rows {start}-{end}")
            ...
            return list(range(zero_based_start, zero_based_end + 1))
    ...
    def get_cluster_centroids(self, df: pd.DataFrame) -> Dict[int, Tuple[float, float, float]]:
        ...
        for cluster in clusters:
            try:
                self.logger.info(f"🔍 Processing cluster: {cluster} (type: {type(cluster)})")
                if not pd.isna(cluster):
                    ...
                    if not cluster_points.empty:
                        ...
                        # DEBUG: Show sample data for this cluster
                        sample_row = cluster_points.iloc[0]
                        self.logger.info(f"  Sample row for cluster {cluster_int}: Centroid_X={sample_row.get(centroid_x_col)}, Centroid_Y={sample_row.get(centroid_y_col)}, Centroid_Z={sample_row.get(centroid_z_col)}")
    ...
    def _calculate_and_save_delta_e_dataframe_mode(self, start_row: int, end_row: int) -> None:
        """
        Calculate ΔE CIE2000 values in DataFrame-only mode (internal worksheet).
        ...
        """
        try:
            self.logger.info("Calculating ΔE values in DataFrame-only mode")
            ...
            # Check if any clusters are assigned
            if self.data['Cluster'].isna().all():
                raise ValueError("No cluster assignments found. Please run K-means clustering first.")
            
            # Check for centroid data
            centroid_cols = ['Centroid_X', 'Centroid_Y', 'Centroid_Z']
            has_centroid_columns = all(col in self.data.columns for col in centroid_cols)
            
            if not has_centroid_columns:
                raise ValueError("Centroid columns not found. Please run K-means clustering and save the results first.")
                
            # Check if at least one row has valid centroid data
            valid_centroid_mask = (
            ...
            # ENHANCED DEBUG: Show exactly what we're working with
            self.logger.info(f"🔍 ΔE DEBUGGING: User specified rows {start_row}-{end_row}")
            self.logger.info(f"🔍 DataFrame has {len(self.data)} total rows")
            self.logger.info(f"🔍 Row indices to process: {row_indices[:10]}{'...' if len(row_indices) > 10 else ''}")
            
            # Get data for our selected range
            subset_data = self.data.iloc[row_indices]
            self.logger.info(f"🔍 Subset data shape: {subset_data.shape}")
            
            # Show cluster distribution in the DataFrame
            cluster_counts = self.data['Cluster'].value_counts().to_dict()
            self.logger.info(f"🔍 Total cluster distribution in DataFrame: {cluster_counts}")
            
            # Get cluster assignments for each point
            clusters = subset_data['Cluster']
            valid_clusters = clusters[clusters.notna()]
            
            self.logger.info(f"🔍 Clusters in selected range: {len(clusters)} total, {len(valid_clusters)} with assignments")
            if len(valid_clusters) > 0:
                subset_cluster_counts = valid_clusters.value_counts().to_dict()
                self.logger.info(f"🔍 Cluster distribution in selected range: {subset_cluster_counts}")
            
            if valid_clusters.empty:
                self.logger.error(f"❌ No cluster assignments found for rows {start_row}-{end_row}")
                self.logger.error(f"❌ Check that K-means was run on the same row range: {start_row}-{end_row}")
                raise ValueError(f"No cluster assignments found for rows {start_row}-{end_row}")
                
            # Calculate ΔE values
            delta_e_values = []
            successful_calculations = 0
            
            self.logger.info(f"Calculating ΔE for {len(valid_clusters)} points with cluster assignments")
            
            # First, build a lookup table of centroids from the centroid rows
            centroids = self.get_cluster_centroids(self.data)
            
            if not centroids:
                raise ValueError("No cluster centroids found. Please run K-means clustering and save results first.")
            
            self.logger.info(f"📍 Found centroids for clusters: {list(centroids.keys())}")
            for cluster_id, (cx, cy, cz) in centroids.items():
                self.logger.info(f"  Cluster {cluster_id}: ({cx:.4f}, {cy:.4f}, {cz:.4f})")
            ...
            for idx_in_subset, row_idx in enumerate(row_indices):
                ...
                if pd.notna(cluster):
                    ...
                    point_x = row.get('Xnorm')
                    point_y = row.get('Ynorm') 
                    point_z = row.get('Znorm')
                    
                    # Look up the centroid coordinates from the centroids table
                    cluster_int = int(cluster)
                    if cluster_int in centroids:
                        centroid_x, centroid_y, centroid_z = centroids[cluster_int]
                    else:
                        self.logger.warning(f"🔍 No centroid found for cluster {cluster_int} in row {row_idx}")
                    ...
                    if all(pd.notna(val) for val in [point_x, point_y, point_z, centroid_x, centroid_y, centroid_z]):
                        try:
                            ...
                            if successful_calculations <= 5:  # Debug first 5 successful calculations
                                self.logger.info(f"✅ CALCULATED ΔE for row {row_idx}: {delta_e:.4f}")
                            
                        except Exception as e:
                            self.logger.warning(f"Failed to calculate ΔE for row {row_idx}: {e}")
                            self.data.at[row_idx, '∆E'] = None
                    else:
                        # Enhanced debugging for missing data
                        if idx_in_subset < 10:  # Only debug first 10 missing cases
                            missing_parts = []
                            if pd.isna(point_x): missing_parts.append("Xnorm")
                            if pd.isna(point_y): missing_parts.append("Ynorm") 
                            if pd.isna(point_z): missing_parts.append("Znorm")
                            if pd.isna(centroid_x): missing_parts.append("Centroid_X")
                            if pd.isna(centroid_y): missing_parts.append("Centroid_Y")
                            if pd.isna(centroid_z): missing_parts.append("Centroid_Z")
                            self.logger.warning(f"🔍 Row {row_idx} missing: {missing_parts} (point: {point_x}, {point_y}, {point_z}, centroid: {centroid_x}, {centroid_y}, {centroid_z})")
                        self.data.at[row_idx, '∆E'] = None
                else:
                    self.logger.debug(f"No cluster assignment for row {row_idx}")
                    self.data.at[row_idx, '∆E'] = None
            ...
            completion_msg = (
                f"Successfully calculated ΔE values for rows {start_row}-{end_row}!\n\n"
                f"Total points processed: {len(row_indices)}\n"
                f"Successful calculations: {successful_calculations}\n"
                f"Failed calculations: {len(row_indices) - successful_calculations}\n\n"
                f"Data has been updated in the internal worksheet."
            )
            
            messagebox.showinfo("ΔE Calculation Complete", completion_msg)
            self.logger.info(f"Successfully calculated {successful_calculations} ΔE values in DataFrame mode")
            
        except Exception as e:
            self.logger.error(f"Error calculating ΔE in DataFrame mode: {e}")
            messagebox.showerror("Error", 
                f"Failed to calculate ΔE values in DataFrame mode:\n\n{str(e)}")
            raise
    ...
    def calculate_and_save_delta_e(self, start_row: int, end_row: int) -> None:
        ...
        try:
            ...
            if self.file_path is None:
                ...
                return self._calculate_and_save_delta_e_dataframe_mode(start_row, end_row)
