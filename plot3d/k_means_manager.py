...
class KmeansManager:
    ...
    def load_data(self, dataframe: pd.DataFrame) -> None:
        ...
        if '_original_sheet_row' not in dataframe.columns:
            # External file mode - clear existing centroid data to start fresh
            dataframe['Centroid_X'] = float('nan')
            dataframe['Centroid_Y'] = float('nan')
            dataframe['Centroid_Z'] = float('nan')
            dataframe['Cluster'] = None
            self.logger.info("Cleared any existing centroid data in the loaded DataFrame (external file mode)")
        else:
            # Realtime mode - preserve existing centroid data from database
            # Check if we have existing centroid data
            has_centroids = (
                'Centroid_X' in dataframe.columns and 
                'Centroid_Y' in dataframe.columns and 
                'Centroid_Z' in dataframe.columns and
                dataframe[['Centroid_X', 'Centroid_Y', 'Centroid_Z']].notna().any().any()
            )
            
            if has_centroids:
                self.logger.info("Preserving existing centroid data from database (realtime mode)")
            else:
                # Initialize columns if they don't exist but don't clear existing data
                if 'Centroid_X' not in dataframe.columns:
                    dataframe['Centroid_X'] = float('nan')
                if 'Centroid_Y' not in dataframe.columns:
                    dataframe['Centroid_Y'] = float('nan')
                if 'Centroid_Z' not in dataframe.columns:
                    dataframe['Centroid_Z'] = float('nan')
                if 'Cluster' not in dataframe.columns:
                    dataframe['Cluster'] = None
                self.logger.info("Initialized centroid columns (realtime mode, no existing data)")
